...
def find_best_match(patch: List[float], strip: List[float]) -> int:
    # TODO: Find patch in strip and return column index (x value) of topleft
    # corner
    best_match = 0
    min_diff = float('inf')
...
# Adapted index values to approximate the difference with the original
# images shapes
patch_loc = [94, 119]
patch_size = [100, 100]
...
# Now look for the patch in the strip and report the best position (column
# index of topleft corner)
...
cv2.imshow('Found patch', patch_right)
cv2.waitKey(0)
