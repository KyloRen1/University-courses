...
def normalize(img_in: List[float]) -> List[float]:
...
def select_gdir(
        ...
        mag_min: int,
        angle_low: int,
        angle_high: int) -> List[float]:
    # TODO: Find and return pixels that fall within the desired mag, angle
    # range
    res = np.logical_and(gmag >= mag_min, gdir >= angle_low)
    res = np.logical_and(res, gdir <= angle_high)
...
# See
# https://sourceforge.net/p/octave/image/ci/default/tree/inst/imgradient.m#l61
