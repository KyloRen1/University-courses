...
def LoG(size: int, sigma: float) -> List[float]:
...
# Make a blurred/smoothed version. Use cv2.getGaussianKernel to get the h
# kernel
# TODO: change this line with the call to cv2.getGaussianKernel
h = cv2.getGaussianKernel(ksize=5, sigma=2)
h = np.multiply(h.T, h)
...
# Use cv2.filter2D with BORDER_CONSTANT to get results similar to the
# Matlab demo
...
# Let's use np.copy to avoid modifying the original array
lenaL = np.copy(lenaSmooth)
# TODO: use numpy indexing to copy and paste the array to the right position
lenaL = np.roll(lenaL, 1, axis=1)[:, 1:]

# Let's use np.copy to avoid modifying the original array
lenaR = np.copy(lenaSmooth)
# TODO: use numpy indexing to copy and paste the array to the right position
lenaR = np.roll(lenaR, -1, axis=1)[:, :-1]

# TODO: Subtract lenaL from lenaR. Don't forget about using the correct
# data type
lenaDiff = lenaL - lenaR  # Change this line with the answer

# Here we shift the value range to fit [0, 255] and make sure the data
# type is uint8 in order to display the results.
lenaDiff = cv2.normalize(
    lenaDiff,
    None,
    alpha=0,
    beta=255,
...
# TODO: use cv2.Canny with lenaMono and the thresholds defined above
cannyEdges = cv2.Canny(lenaMono, thresh1, thresh2)
cv2.imshow('Original edges', cannyEdges)

# TODO: use cv2.Canny with lenaSmooth and the thresholds defined above
...
logEdges = cv2.filter2D(cannyEdges, -1, h)  # TODO: use cv2.filter2D
logEdgesShow = cv2.normalize(
    logEdges,
    None,
    alpha=0,
    beta=255,
...
cv2.imshow(
    'Laplacian image before zero crossing',
    logEdgesShow.astype(
        np.uint8))

# OpenCV doesn't have a function edge like Matlab that implements a 'log' method. This would
# have to be implemented from scratch. This may take a little more time to
# implement this :).
